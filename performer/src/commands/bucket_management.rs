...
impl BucketManagerCommand {
    pub fn new(
        ...
        let timeout_override = match &command {
            Command::GetBucket(cmd) => cmd.options.as_ref().and_then(|o| o.timeout_msecs),
            Command::GetAllBuckets(cmd) => cmd.options.as_ref().and_then(|o| o.timeout_msecs),
            Command::CreateBucket(cmd) => cmd.options.as_ref().and_then(|o| o.timeout_msecs),
            Command::DropBucket(cmd) => cmd.options.as_ref().and_then(|o| o.timeout_msecs),
            Command::FlushBucket(cmd) => cmd.options.as_ref().and_then(|o| o.timeout_msecs),
            Command::UpdateBucket(cmd) => cmd.options.as_ref().and_then(|o| o.timeout_msecs),
        }
