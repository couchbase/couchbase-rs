...
use crate::commands::kv::{
    ...
    InsertCommand, KvCommand, KvCommandKind, RemoveCommand, ReplaceCommand, TouchCommand,
    UnlockCommand, UpsertCommand,
};
...
fn build_get_replica_command(
    ...
    let timeout_override =
        timeout_override_millis(get_replica.options.as_ref().and_then(|o| o.timeout_msecs))?;

    Ok(SdkCommand::KV(KvCommand::new(
        KvCommandKind::GetReplica(GetReplicaCommand::new(
            collection,
            doc_location.id().to_string(),
            strategy,
            return_result,
